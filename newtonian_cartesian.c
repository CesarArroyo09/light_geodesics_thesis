...
#define M 5000.0     //Mass of the perturbation
...
mydbl geodesic_equation_0(mydbl p0, mydbl p1, mydbl p2, mydbl p3, mydbl t, mydbl x1, mydbl x2, mydbl x3)
{
  mydbl f = - (2/powl(C,2))*p0*(p1*ith_der_potential(x1, x2, x3) + p2*ith_der_potential(x2, x1, x3) + p3*ith_der_potential(x3, x2, x1));
  return f;
}
...
mydbl geodesic_equation_i(mydbl p0, mydbl pi, mydbl pj1, mydbl pj2, mydbl t, mydbl xi, mydbl xj1, mydbl xj2)
{
  mydbl f =  -ith_der_potential(xi,xj1,xj2)*powl(p0,2)  + (1/powl(C,2))*(ith_der_potential(xi,xj1,xj2)*powl(pi,2) - ith_der_potential(xj1,xi,xj2)*powl(pj1,2) - ith_der_potential(xj2,xj1,xi)*powl(pj2,2)) + (2/powl(C,2))*(ith_der_potential(xj1,xi,xj2)*pi*pj1 + ith_der_potential(xj2,xi,xj1)*pi*pj2);
  return f;
}

void euler1(mydbl *t, mydbl *x1, mydbl *x2, mydbl *x3, mydbl *p0, mydbl *p1, mydbl *p2, mydbl *p3, mydbl *lambda)
{
  /*Increment in the variables of the differential equation we want to solve*/
  mydbl dt, dx1, dx2, dx3, dp0, dp1, dp2, dp3;
...
int main(void)
{
  ...
  /*Initial conditions*/
  mydbl t = 0.0, x1 = 0.0, x2 = 0.0, x3 = 0.0, p0 = 1.0e-1, p1, p2 = 0.0, p3 = 0.0, lambda = 0.0, nu = 0.0;
